...
export function FilterBar({ zipCode, sortBy, dateFrom, dateTo, onChange }: FilterBarProps) {
  ...
  return (
    <section className="sticky top-20 z-40 mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-5">
      <div className="mb-4 flex items-center gap-2 text-sm font-medium text-slate-700">
        <SlidersHorizontal className="h-4 w-4 text-slate-500" />
        Filters
        {activeFilters ? <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600">{activeFilters} active</span> : null}
      </div>
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-12">
        <label className="relative block xl:col-span-3">
          ...
          <input
            ...
            className="min-h-11 w-full rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-base text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-300 md:text-sm"
          />
        </label>
        <label className="relative block xl:col-span-3">
